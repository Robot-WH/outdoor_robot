...
void *SerialRecvTask(void *arg)
{
	...
	while (1)
	{
		...
		{
			printf("RECV=%x\n", buff[i]);
			if ((RECV_PACK_START_FLAG == (unsigned char)buff[i]) && (RECV_PACK_START_FLAG_RT == (unsigned char)buff[i + 1]) && (RT_RECV_PACK_LEN <= (count - i)))
			{
				iIndex = i;
				bIsValidPkg = true;
				break;
			}
			else if ((RECV_PACK_START_FLAG == (unsigned char)buff[i]) && (RECV_PACK_START_FLAG_NRT == (unsigned char)buff[i + 1]) && (NRT_RECV_PACK_LEN <= (count - i)))
			{
				iIndex = i;
				bIsValidPkg = true;
				break;
